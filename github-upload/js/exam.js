...
const Exam = (() => {
  ...
  function startExam() {
    ...
    if (state.questions.length === 0) {
      window.App?.showToast('⚠️ No PYQs available for the selected topic/unit yet. Try another one!', 'warning');
      return;
    }
    state.current = 0;
    state.answers = [];
    ...
    el.innerHTML = `
      <div class="exam-page">
        ...
        <div class="exam-setup">
          ...
          <div id="exam-unit-selector" class="${state.mode === 'unit' ? '' : 'hidden'}" style="margin-bottom:1rem;">
            <label style="font-size:0.875rem;font-weight:600;display:block;margin-bottom:0.5rem;">Select Unit:</label>
            <div style="display:flex;gap:0.5rem;flex-wrap:wrap;">
              ${[1,2,3,4].map(u => `
                <button class="btn btn-sm unit-select-btn ${state.unit === u ? 'btn-primary' : 'btn-outline'}" data-unit="${u}">Unit ${u}</button>
              `).join('')}
            </div>
          </div>

          <div id="exam-topic-selector" class="${state.mode === 'topic' ? '' : 'hidden'}" style="margin-bottom:1rem;">
            ...
            <select id="topic-select" style="width:100%;padding:0.625rem 0.875rem;background:var(--bg-elevated);border:1px solid var(--border);border-radius:var(--radius-sm);color:var(--text-primary);font-size:0.875rem;outline:none;">
              ${TOPICS.map(t => `<option value="${t.id}" ${state.topic === t.id ? 'selected' : ''}>Unit ${t.unit}: ${t.title}</option>`).join('')}
            </select>
          </div>
  ...
  function renderScoreCard() {
    ...
    // Build wrong answers review with chosen answers
    const wrongAnswers = state.questions.map((q, i) => ({ q, chosen: state.answers[i] }))
      ...
      <div class="exam-page">
        ...
        ${wrongAnswers.length ? `
          <div class="card" style="margin-top:1.5rem;text-align:left;">
            <h4 style="margin-bottom:1.25rem;color:#f87171;font-weight:700;">❌ Review Wrong Answers (${wrongAnswers.length})</h4>
            ${wrongAnswers.map(({ q, chosen }) => {
              const correctText = q.options.find(o => o.startsWith(q.answer))?.replace(/^[A-D]\)\s*/, '') || '';
              const chosenText = chosen === 'TIMEOUT' ? 'Time Out ⏱'
                : chosen === undefined ? 'Skipped'
                : q.options.find(o => o.startsWith(chosen))?.replace(/^[A-D]\)\s*/, '') || chosen;
              return `
                <div class="pyq-question-card" style="margin-bottom:1.25rem;padding:1.25rem;background:var(--bg-elevated);border:1px solid var(--border);border-radius:var(--radius-md);">
                  <div class="pyq-question-text" style="font-weight:600;margin-bottom:0.75rem;line-height:1.45;">${q.question}</div>
                  <div style="display:flex;gap:0.5rem;flex-wrap:wrap;margin-bottom:0.75rem;">
                    <span class="tag tag-red" style="background:rgba(239,68,68,0.1);color:#ef4444;border:1px solid rgba(239,68,68,0.2);padding:0.25rem 0.625rem;font-size:0.75rem;border-radius:4px;">
                      Your Choice: (${chosen === 'TIMEOUT' ? '⏱' : chosen}) ${chosenText}
                    </span>
                    <span class="tag tag-green" style="background:rgba(16,185,129,0.1);color:#10b981;border:1px solid rgba(16,185,129,0.2);padding:0.25rem 0.625rem;font-size:0.75rem;border-radius:4px;">
                      Correct: (${q.answer}) ${correctText}
                    </span>
                  </div>
                  <div style="font-size:0.825rem;color:var(--text-secondary);margin-top:0.50rem;padding-top:0.50rem;border-top:1px solid var(--border);line-height:1.45;">
                    <strong>Explanation:</strong> ${q.explanation}
                  </div>
                </div>
              `;
